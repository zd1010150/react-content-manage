...
export const tryFetchRelatedTos = (id, type, taskId, taskData) => dispatch =>
  get(`/admin/tasks/related-object-options/${type}/${id}`, {}, dispatch).then((data) => {
    if (data && _.isArray(data.data)) {
      dispatch(setRelatedTos(data.data));
      if (taskId !== PhantomId) {
        dispatch(setRelatedTo(`${taskData.taskable_type}__${taskData.relate_user.id}`));
      } else {
        dispatch(setRelatedTo(`${data.data[0].taskable_type}__${data.data[0].taskable_id}`));
      }
    }
  });
...
export const tryFetchTask = (taskId, objectId, objectType, defaultStateId = '') => dispatch =>
  get(`/admin/tasks/${taskId}`, {}, dispatch).then((data) => {
    if (data && !_.isEmpty(data.data)) {
      ...
      dispatch(tryFetchRelatedTos(objectId, objectType, taskId, data.data));
    }
  });
