...
const DepartmentComponent = ({
  ...
  const actionsRight = (<div><Button
    className="btn-ellipse email-theme-btn"
    size="small"
    onClick={() => {
        setDepartmentVisible(!isDepartmentVisible);
    }}
  ><Icon type="edit" />{ formatMessage({ id: 'page.emailTemplates.hideDepartments' }) }
                             </Button>
                        </div>);
  return (
    <Panel
      panelTitle={formatMessage({ id: 'page.emailTemplates.emailTemplates' })}
      panelClasses="email-theme-panel"
      contentClasses="pl-lg pr-lg"
